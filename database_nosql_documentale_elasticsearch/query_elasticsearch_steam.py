...
for doc in risultato_2["hits"]["hits"]:
    # Stampo l'username e il testo integrale
...
print("\n[*] Esecuzione terminata.")            "created_at":       { "type": "date", "format": "yyyy-MM-dd" }
        }
    }
}
...
# Estraiamo l'array dei documenti che matchano la ricerca navigando il JSON di risposta: hits -> hits
for doc in risultato_2["hits"]["hits"]:
    # Stampo l'username e un'anteprima testuale troncata
    print(f"[{doc['_source']['username']}] -> {doc['_source']['review_text'][:90]}...")
...
risultato_4 = es.search(index=indice, body=query_4)
for doc in risultato_4["hits"]["hits"]:
    print(f"- {doc['_source']['username']}: {doc['_source']['review_text'][:80]}...")
